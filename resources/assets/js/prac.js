...
Vue.component('node',{
	...
	<button @click="tentativeattack" class="btn node" v-bind:class="classobject">
		...
		<div class="progress">
  		...
  		<div class="progress-bar bg-success" role="progressbar" v-bind:style="styleobjectValue"  aria-valuemin="0" aria-valuemax="100"></div>
		<div style="padding=2px;">V: {{nodevalue}}</div>
	</div>
	<div class="progress">
	  
	  <div class="progress-bar bg-danger" role="progressbar" v-bind:style="styleobjectCost"  aria-valuemin="0" aria-valuemax="100"></div>
		<div style="padding=2px;">C: {{cost}}</div>
	</div>
	<div class="progress">
		 
 		 <div class="progress-bar bg-warning" role="progressbar" v-bind:style="styleobjectTime"  aria-valuemin="0" aria-valuemax="100"></div>
		<div style="padding=2px;">T: {{timerequired}}</div>
	</div>
	...
	data : {
		...
		currentattackset : [], 
		possibleattackset : [0,1], // initially only the public nodes
		public : [0,1],
	...
	methods : {
			...
			date: function (date) 
			{
	   		...
	   		confirmAttack : function()
	   		{
	   			...
	   			if(vm.attacker_tentative_move !== '')
			     {
			      	...
			      	EventListeners.$emit('attackerMovedconfirmed', vm.attacker_tentative_move, vm.newattackneighbors, vm.date(Date.now()));
			      }
			      	...
			      	if(vm.timer>0)
			        {
			      	...
			      	if((vm.defendermoved==true && vm.attackermoved==true) || (vm.attacker_tentative_move !== '' && vm.timer==0))
			      	{
			      		...
			      		if(vm.timer==0)
			      		{
							...
							vm.gamehistory.time_attacker_moved = vm.date(Date.now());//(vm.tentative_time_attacker_moved);
			...
			makeDefenderMove : function()
			{
				...
				// select a random node
				 ...
				 console.log('Defender action $$$$$$$$ '+ vm.defenderaction + ' counter  '+ vm.defstrategycounter);
	...
	created () {
		...
		EventListeners.$on('bothmoved', function(defenderaction, attackeraction){
			...
			if (vm.gamehistory.attacker_action < 5 && vm.gamehistory.attacker_action !== '') {
				$('<p style="font-size: 70%;">Round ' + vm.numberofround + ': <span style="color: red"> You ' + vm.gamehistory.attacker_action + '</span>, <span style="color: green">defender ' + vm.defenderaction + '</span></p>').appendTo('#log');
			} else if (vm.gamehistory.attacker_action == 5) {
				$('<p style="font-size: 70%;">Round ' + vm.numberofround + ': <span style="color: red">You PASS</span>, <span style="color: green">defender ' + vm.defenderaction + '</span></p>').appendTo('#log');
			} else {
				$('<p style="font-size: 70%;">Round ' + vm.numberofround + ': <span style="color: red">You NONE</span>, <span style="color: green">defender ' + vm.defenderaction + '</span></p>').appendTo('#log');
			}
